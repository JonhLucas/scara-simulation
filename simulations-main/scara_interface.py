...
class ScaraSimul(Node):
    def __init__(self, robot_handle, joints_handles, end_effector_handle):
        ...
        self.place_service = self.create_service(GrabOrRelease, 'scara/goal_place', self.place_service_callback)
        self.place_service
    ...
    def grab_service_callback(self, request, response):
        ...
        if request.command == 0:
            ...
            #r = sim.getObjectParent(goal)
            #param = sim.getObjectInt32Param(goal, 3003)

            sim.setObjectInt32Parameter(goal, 3003, 1)
            sim.resetDynamicObject(goal)
            sim.setObjectParent(goal, endEffector, True)
        
            #self.get_logger().info("Service: " + str(r) + " \n")

        elif request.command == 1:
            sim.setObjectInt32Parameter(goal, 3003, 0)
            sim.resetDynamicObject(goal)
            sim.setObjectParent(goal, -1, True)

        return response
    
    def place_service_callback(self, request, response):
